    ...
    (() => {
        ...
        function formFieldsInit(options = {
            ...
            if (options.inputValidate) document.body.addEventListener("input", (function(e) {
                const targetElement = e.target;
                if ("INPUT" === targetElement.tagName && ("email" !== targetElement.dataset.required || "" !== targetElement.dataset.required)) formValidate.inputValidate(targetElement, true);
            }));
        ...
        let formValidate = {
            getErrors(form) {
                ...
                if (formRequiredItems.length) formRequiredItems.forEach((formRequiredItem => {
                    if ((null !== formRequiredItem.offsetParent || "SELECT" === formRequiredItem.tagName) && !formRequiredItem.disabled) if ("" !== formRequiredItem.dataset.required && "email" !== formRequiredItem.dataset.required) error += this.validateInput(formRequiredItem) + this.inputValidate(formRequiredItem, true); else error += this.validateInput(formRequiredItem);
                }));
                return error;
            },
            ...
            inputValidate(formRequiredItem, start = false) {
                ...
                if (start) {
                    if ("text" === formRequiredItem.dataset.required) if (!formRequiredItem.value.match(/^[a-zа-яёіїє\s]*$/iu)) {
                        ...
                        this.addInputError(formRequiredItem, "Некорректний номер телефону");
                        error++;
                    } else {
                        this.removeInputError(formRequiredItem);
                        error >= 1 ? error = 0 : null;
        ...
        function initSliders() {
            if (document.querySelector(".feedback__slider")) new core(".feedback__slider", {
                ...
                speed: 800,
                navigation: {
                    prevEl: ".arrows__arrow_prev",
                    nextEl: ".arrows__arrow_next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1.3,
        ...
        da.init();
        const lines = document.querySelectorAll(".form-order__line");
        if (lines.length) lines.forEach((line => {
            const input = line.querySelector("input");
            const label = line.querySelector("label");
            if (input && label) input.addEventListener("blur", (function() {
                if ("" !== input.value) label.style.cssText = `\n\t\t\t\t\ttop: 0;\n\t\t\t\t\tleft: 50%;\n\t\t\t\t\tfont-size: 15px;\n\t\t\t\t\ttransform: translate(-50%, -60%);\n\t\t\t\t\t`; else label.style.cssText = null;
            }));
        }));
        window["FLS"] = true;
        isWebp();
